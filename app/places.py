        ...
        async with httpx.AsyncClient(timeout=TIMEOUT) as c:
            for q in queries:
                ...
                for p in places:
                    ...
                    # Se acepta con: web coincidente (fiable al 100%) O nombre muy parecido
                    # EN LA MISMA CIUDAD que detectamos del sitio (muchas fichas legítimas
                    # solo tienen el nombre, sin web; la misma ciudad la hace casi seguro
                    # suya) siempre que la ficha no enlace OTRA web distinta. Sin ciudad
                    # coincidente NO aceptamos por nombre (evita el cruce entre ciudades).
                    accept = same_web or (sim >= 0.85 and same_city and not web_conflict)
