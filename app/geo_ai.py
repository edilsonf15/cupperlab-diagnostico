...
async def run_ai_geo_fast(domain: str, meta: dict, lang: str = "es") -> dict | None:
    ...
    if lang == "en":
        q_mega = (
            ...
            f"COMPETENCIA: <ALWAYS give 4-6 of the BEST-KNOWN, RECOGNIZABLE brands a customer would consider as a "
            f"DIRECT alternative to \"{brand}\" in its country/market (same country as ZONA) and its exact segment "
            f"(e.g. for a women's fashion brand in Colombia: Studio F, Koaj, Arturo Calle, Chevignon, Americanino, "
            f"Tennis, Pilatos...). Real, well-known brands of a similar type and level, present in that country. "
            f"Do NOT list tiny unknown shops, and do NOT list generic global giants (Amazon, Shein) if there are "
            f"leading local/regional brands. Never leave this empty; separated by |>\n"
        ...
        q_mega = (
            ...
            f"COMPETENCIA: <SIEMPRE da 4-6 de las marcas MÁS CONOCIDAS y RECONOCIBLES que un cliente consideraría como "
            f"alternativa DIRECTA a \"{brand}\" en su país/mercado (el mismo país que ZONA) y en su MISMO segmento "
            f"(p. ej. para una marca de ropa femenina en Colombia: Studio F, Koaj, Arturo Calle, Chevignon, "
            f"Americanino, Tennis, Pilatos...). Marcas reales y reconocidas, del mismo tipo y nivel, presentes en ese "
            f"país. NO pongas tiendas diminutas o desconocidas, y NO pongas gigantes globales genéricos (Amazon, "
            f"Shein) si hay marcas líderes locales/regionales. Nunca lo dejes vacío; separadas por |>\n"
