...
async def run_ai_geo_fast(domain: str, meta: dict, lang: str = "es") -> dict | None:
    ...
    if lang == "en":
        q_mega = (
            ...
            f"COUNTRY/MARKET where this website operates (the same country as ZONA). Prioritize competitors present "
            f"in that country (local or regional); avoid listing only global giants if there are relevant local/regional "
            f"alternatives. Never leave this empty; separated by |>\n"
        ...
        q_mega = (
            ...
            f"PAÍS/MERCADO donde opera esta web (el mismo país que ZONA). Prioriza competidores presentes en ese país "
            f"(locales o regionales); evita listar solo gigantes globales si existen alternativas locales/regionales "
            f"relevantes. Nunca lo dejes vacío; separadas por |>\n"
